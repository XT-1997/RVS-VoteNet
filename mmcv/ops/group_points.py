...
class QueryAndGroup(nn.Module):
    ...
    def forward(self,
        ...
        if features is not None:
            ...
            # grouped_features = torch.cat([grouped_features, grouped_xyz], dim=1).permute(0,2,3,1).contiguous()
            grouped_features = torch.cat([grouped_xyz, grouped_features],
                                         dim=1).permute(0, 2, 3,
                                                        1).contiguous()

            if self.use_xyz:
                mean = torch.cat([center_xyz, sam_features], dim=-1).unsqueeze(
                    dim=-2)  # [B, npoint, 1, d+3]
                std = torch.std(
                    (grouped_features - mean).reshape(sam_features.size(0),
                                                      -1),
                    dim=-1,
                    keepdim=True).unsqueeze(dim=-1).unsqueeze(dim=-1)
                grouped_features = (grouped_features - mean) / (std + 1e-5)
                new_features = (self.affine_alpha * grouped_features +
                                self.affine_beta)

                new_features = torch.cat([
                    center_xyz.view(sam_features.size(0), sam_features.size(1),
                                    1, -1).repeat(1, 1, self.sample_num, 1),
                    new_features,
                    sam_features.view(sam_features.size(0),
                                      sam_features.size(1), 1, -1).repeat(
                                          1, 1, self.sample_num, 1)
                ],
                                         dim=-1).permute(0, 3, 1,
                                                         2).contiguous()
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor,
        ...
        ext_module.group_points_forward(features,
                                        indices,
                                        output,
                                        b=B,
                                        c=C,
    ...
    def backward(ctx,
        ...
        ext_module.group_points_backward(grad_out_data,
                                         idx,
                                         grad_features.data,
                                         b=B,
                                         c=C,
