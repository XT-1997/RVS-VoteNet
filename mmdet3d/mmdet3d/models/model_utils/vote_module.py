...
class VoteModule(nn.Module):
    ...
    def get_loss(self, seed_points, vote_points, seed_indices,
                 vote_targets_mask, vote_targets, offset, pts_semantic_mask):
        ...
        non_weight = (torch.ones_like(seed_gt_votes_mask) - seed_gt_votes_mask) / torch.sum(torch.ones_like(seed_gt_votes_mask) - seed_gt_votes_mask + 1e-6)
        non_vote_loss = torch.sum(torch.norm(offset, p=1, dim=1) * non_weight) 

        return vote_loss + non_vote_loss   # vote_loss + 0.5 * non_vote_loss
